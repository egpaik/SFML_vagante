...
void JoystickImpl::initializeDInput()
{
    ...
    if (dinput8dll)
    {
        ...
        {
            // NOTE in case we try to sync w/ SFML again in the future:
            //  For some reason, SFML's DirectInput implementation causes
            //  Vagante to CRASH on exit when the directInput handle gets
            //  released - therefore I am just going to comment out this
            //  initialization, effectively disabling DirectInput in SFML's
            //  internal state.  ~K6L2 14/8/2019
